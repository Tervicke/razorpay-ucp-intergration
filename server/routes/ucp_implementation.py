...
async def complete_checkout(
  ...
  # The UCP request body contains required `payment` and optional `signals`.
  # Parse the entire body as one model so omitting signals remains valid.
  payment_req = PaymentCreateRequest(
    **checkout_complete.payment.model_dump(mode="json", by_alias=True)
  )
  signals = (
    checkout_complete.signals.model_dump(
      mode="json", by_alias=True, exclude_none=True
    )
    if checkout_complete.signals
    else {}
  )
