...
class CheckoutService:
  ...
  async def get_checkout(
    ...
    await self.transactions_session.commit()
    reconciled = await self._reconcile_razorpay_payment_link(checkout_id)
    if reconciled:
      return reconciled
  ...
  async def complete_checkout(
    ...
    handler_id = (
      payment.instruments[0].handler_id if payment.instruments else None
    )
    if handler_id == "razorpay_mock_payment_handler":
      return await self._prepare_razorpay_payment(
        checkout, payment, idempotency_key, request_hash
      )
  ...
  @staticmethod
  def _validate_checkout_fulfillment(checkout: Checkout) -> None:
    """Ensure a selected destination and fulfillment option are present."""
    for method in checkout.fulfillment.methods if checkout.fulfillment else []:
      if method.type == "shipping" and not method.selected_destination_id:
        continue
      if any(group.selected_option_id for group in (method.groups or [])):
        return
    raise InvalidRequestError(
      "Fulfillment address and option must be selected before completion."
    )
  ...
  @staticmethod
  def _checkout_total(checkout: Checkout) -> int:
    """Read the authoritative final total calculated by the business."""
    totals = [
      total.amount for total in checkout.totals if total.type == "total"
    ]
    if len(totals) != 1 or totals[0] < 0:
      raise InvalidRequestError("Checkout has no valid final total")
    return totals[0]

  async def _prepare_razorpay_payment(
    self,
    checkout: Checkout,
    payment: PaymentCreateRequest,
    idempotency_key: str,
    ...
    """Reserve stock and create a Razorpay order without completing checkout."""
    del payment  # Handler selection was already validated by the caller.
    self._validate_checkout_fulfillment(checkout)
    await self._release_expired_reservations()

    existing = await db.get_pending_payment_attempt(
      self.transactions_session, checkout.id
    )
    if existing:
      return checkout
    ...
    try:
      for line in checkout.line_items:
        if not await db.reserve_stock(
          self.transactions_session, line.item.id, line.quantity
        ):
          raise OutOfStockError(
            f"Insufficient stock for item {line.item.id}", status_code=409
          )
        items.append({"product_id": line.item.id, "quantity": line.quantity})

      now = datetime.datetime.now(datetime.UTC)
      # Razorpay requires expire_by to remain at least 15 minutes in the
      # future when its API receives the request. This Razorpay account rejects
      # shorter values, so keep a one-hour minimum and align the inventory hold
      # with the hosted link.
      hold_seconds = max(
        config.FLAGS["payment_reservation_ttl_seconds"].value, 60 * 60
      )
      reservation_expires_at = now + datetime.timedelta(seconds=hold_seconds)
      reservation = db.InventoryReservation(
        id=reservation_id,
        checkout_id=checkout.id,
        status="active",
        items=items,
      ...
      self.transactions_session.add(reservation)

      provider_link = await RazorpayService().create_payment_link(
        amount=self._checkout_total(checkout),
        currency=checkout.currency,
        checkout_id=checkout.id,
        expire_by=int(reservation_expires_at.timestamp()),
      )
      attempt = db.PaymentAttempt(
        id=f"payatt_{uuid.uuid4()}",
        checkout_id=checkout.id,
        handler_id="razorpay_mock_payment_handler",
        # Before payment_link.paid, this stores the link ID. That event provides
        # the generated Razorpay Order ID and atomically remaps the attempt.
        provider_order_id=provider_link["id"],
        amount=self._checkout_total(checkout),
        currency=checkout.currency,
        status="created",
      ...
      self.transactions_session.add(attempt)

      checkout.status = CheckoutStatus.COMPLETE_IN_PROGRESS
      response_body = checkout.model_dump(
        mode="json", by_alias=True, exclude_none=True
      )
      response_body.setdefault("payment", {})["razorpay"] = {
        "payment_link_id": provider_link["id"],
      ...
      await db.save_checkout(
        self.transactions_session, checkout.id, checkout.status, response_body
      )
      await db.save_idempotency_record(
        self.transactions_session,
        idempotency_key,
  ...
  async def finalize_razorpay_payment(
    ...
    """Finalize a captured Razorpay payment exactly once."""
    attempt = await db.get_payment_attempt_by_provider_order(
      self.transactions_session, provider_order_id
    )
    if not attempt:
      raise ResourceNotFoundError("Unknown Razorpay order")
    checkout = await self._get_and_validate_checkout(attempt.checkout_id)
    if (
      attempt.status == "captured"
      and checkout.status == CheckoutStatus.COMPLETED
    ):
      if attempt.provider_payment_id != provider_payment_id:
        raise PaymentFailedError(
          "Razorpay order was captured by another payment"
        )
      return checkout

    provider_payment = await RazorpayService().fetch_payment(
      provider_payment_id
    )
    if (
      provider_payment.get("status") != "captured"
      or provider_payment.get("captured") is not True
      or provider_payment.get("order_id") != provider_order_id
      or provider_payment.get("amount") != attempt.amount
      or provider_payment.get("currency") != attempt.currency
    ):
      raise PaymentFailedError(
        "Razorpay payment is not captured or mismatches checkout"
      )

    reservation = await self.transactions_session.get(
      db.InventoryReservation, attempt.reservation_id
    )
    if not reservation or reservation.status != "active":
      raise PaymentFailedError("Inventory reservation is no longer active")

    # Re-enter the existing atomic order builder. Releasing and immediately
    # reserving occurs in this same DB transaction, so stock is never exposed.
    for item in reservation.items:
      await db.release_stock(
        self.transactions_session, item["product_id"], item["quantity"]
      )
    reservation.status = "consumed"
    attempt.status = "captured"
    attempt.provider_payment_id = provider_payment_id
    attempt.updated_at = datetime.datetime.now(datetime.UTC).isoformat()
    checkout.status = CheckoutStatus.READY_FOR_COMPLETE
    await db.save_checkout(
      self.transactions_session,
      checkout.id,
      checkout.status,
      checkout.model_dump(mode="json", by_alias=True, exclude_none=True),
    )
    await self.transactions_session.flush()

    confirmed_payment = PaymentCreateRequest(
      instruments=[
        {
          "id": f"rzp_{provider_payment_id}",
    ...
    return await self.complete_checkout(
      ...
      confirmed_payment,
      {},
      f"razorpay-captured:{provider_payment_id}",
    )

  async def confirm_razorpay_payment(
    self, provider_order_id: str, provider_payment_id: str, signature: str
  ) -> Checkout:
    """Verify the browser result and confirm it against Razorpay's API."""
    if not RazorpayService.verify_checkout_signature(
      provider_order_id, provider_payment_id, signature
    ):
      raise InvalidRequestError("Invalid Razorpay checkout signature")
    return await self.finalize_razorpay_payment(
      provider_order_id, provider_payment_id
    )

  async def fail_razorpay_payment(
    self, provider_order_id: str, provider_payment_id: str | None = None
  ) -> None:
    """Release held stock after a provider reports terminal failure."""
    attempt = await db.get_payment_attempt_by_provider_order(
      self.transactions_session, provider_order_id
    )
    if not attempt or attempt.status in {"failed", "captured"}:
      return
    reservation = await self.transactions_session.get(
      db.InventoryReservation, attempt.reservation_id
    )
    ...
    attempt.updated_at = datetime.datetime.now(datetime.UTC).isoformat()
    checkout = await self._get_and_validate_checkout(attempt.checkout_id)
    if checkout.status == CheckoutStatus.COMPLETE_IN_PROGRESS:
      checkout.status = CheckoutStatus.READY_FOR_COMPLETE
      await db.save_checkout(
        self.transactions_session,
        checkout.id,
  ...
  async def _release_expired_reservations(self) -> None:
    """Lazily release expired holds before creating another payment."""
    from sqlalchemy import select

    now = datetime.datetime.now(datetime.UTC).isoformat()
    result = await self.transactions_session.execute(
      select(db.InventoryReservation)
      .where(db.InventoryReservation.status == "active")
      .where(db.InventoryReservation.expires_at <= now)
    )
    for reservation in result.scalars():
      ...
      attempt_result = await self.transactions_session.execute(
        select(db.PaymentAttempt).where(
          db.PaymentAttempt.reservation_id == reservation.id
        )
      )
      attempt = attempt_result.scalar_one_or_none()
      if attempt and attempt.status != "captured":
        attempt.status = "expired"
        attempt.updated_at = now
      data = await db.get_checkout_session(
        self.transactions_session, reservation.checkout_id
      )
      if data and data.get("status") == CheckoutStatus.COMPLETE_IN_PROGRESS:
        data["status"] = CheckoutStatus.READY_FOR_COMPLETE
        await db.save_checkout(
          self.transactions_session,
          reservation.checkout_id,
          CheckoutStatus.READY_FOR_COMPLETE,
          data,
        )
    await self.transactions_session.flush()

  async def _reconcile_razorpay_payment_link(
    self, checkout_id: str
  ) -> Checkout | None:
    """Poll Razorpay and finalize a paid link when no webhook is available."""
    attempt = await db.get_pending_payment_attempt(
      self.transactions_session, checkout_id
    )
    if not attempt or not attempt.provider_order_id.startswith("plink_"):
      return None
    try:
      payment_link = await RazorpayService().fetch_payment_link(
        attempt.provider_order_id
      )
    except PaymentFailedError as exc:
      logger.warning("Unable to reconcile Razorpay link: %s", exc.message)
      return None
    ...
    if not payment_id or not order_id:
      logger.warning(
        "Paid Razorpay link %s has no payment/order identifiers",
        attempt.provider_order_id,
      )
      return None
    ...
    await self.transactions_session.flush()
    return await self.finalize_razorpay_payment(order_id, payment_id)
