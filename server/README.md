...
| `--razorpay_key_id` | unset | Razorpay API key ID used to create and verify provider orders. |
| `--razorpay_key_secret` | unset | Razorpay API secret; also verifies Checkout result signatures. |
| `--razorpay_webhook_secret` | unset | Secret configured for the `/webhooks/razorpay` endpoint. |
| `--payment_reservation_ttl_seconds` | `900` | Inventory hold duration while a Razorpay payment is pending. |

### Razorpay asynchronous completion

Copy `.env.example` from the repository root to `.env` and populate the
Razorpay test credentials. The server loads either `server/.env` or the
repository-root `.env`; command-line flags still override these defaults.

When the selected handler is `razorpay_mock_payment_handler`, Complete Checkout
validates fulfillment, reserves inventory, creates a hosted Razorpay Payment
Link for the server-calculated checkout total, and returns
`complete_in_progress`. The response's `payment.razorpay` extension contains
`payment_url`, the Payment Link ID, amount, and currency. The buyer can open
that URL directly; no custom Razorpay Checkout page is required.

After Razorpay Checkout succeeds, POST its `razorpay_order_id`,
`razorpay_payment_id`, and `razorpay_signature` to
`/payments/razorpay/confirm`. The server verifies the signature and fetches the
payment from Razorpay. Alternatively, configure Razorpay to deliver
`payment_link.paid`, `payment.captured`, `order.paid`, and `payment.failed`
events to
`/webhooks/razorpay`. Only a captured payment with the expected provider order,
amount, and currency creates the merchant order and changes the checkout to
`completed`. Failed or expired attempts release their inventory reservation.
