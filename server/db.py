...
class InventoryReservation(TransactionBase):
  """Inventory held while an asynchronous payment is pending."""
...
class PaymentAttempt(TransactionBase):
  """Persisted state tying a checkout to a Razorpay order/payment."""
  ...
  status = Column(String, index=True)
  reservation_id = Column(String, ForeignKey("inventory_reservations.id"))
  created_at = Column(String)
  updated_at = Column(String)


class ProcessedWebhook(TransactionBase):
  """Webhook event IDs recorded for idempotent provider processing."""
...
async def release_stock(
  ...
  """Return previously reserved stock to inventory."""
...
async def get_payment_attempt_by_provider_order(
  session: AsyncSession, provider_order_id: str
) -> PaymentAttempt | None:
  """Get a payment attempt by its provider order ID."""
  result = await session.execute(
...
async def get_pending_payment_attempt(
  session: AsyncSession, checkout_id: str
) -> PaymentAttempt | None:
  """Get the latest non-terminal payment attempt for a checkout."""
  result = await session.execute(
    select(PaymentAttempt)
    .where(PaymentAttempt.checkout_id == checkout_id)
    .where(PaymentAttempt.status.in_(["created", "authorized"]))
