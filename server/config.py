...
try:
  flags.DEFINE_string(
    "products_db_path", os.getenv("PRODUCTS_DB_PATH"), "Path to products DB"
  )
  flags.DEFINE_string(
    "transactions_db_path",
    os.getenv("TRANSACTIONS_DB_PATH"),
    "Path to transactions DB",
  )
  flags.DEFINE_string(
    "simulation_secret",
    str(uuid.uuid4()),
  ...
  flags.DEFINE_integer(
    "port",
    int(os.environ["PORT"]) if os.getenv("PORT") else None,
  ...
  flags.DEFINE_string(
    "razorpay_key_id", os.getenv("RAZORPAY_KEY_ID"), "Razorpay API key ID"
  )
  flags.DEFINE_string(
    "razorpay_key_secret",
  ...
  flags.DEFINE_string(
    ...
    "Secret used to verify Razorpay webhooks",
  )
  flags.DEFINE_integer(
    "payment_reservation_ttl_seconds",
    int(os.getenv("PAYMENT_RESERVATION_TTL_SECONDS", "900")),
    "Seconds to hold inventory while payment is pending",
    lower_bound=60,
  )
